...
while(eval("window.menu"+m)){mr+="menu"+m+",";tmenu=self["menu"+m];Mname[m]=tmenu[0].toLowerCase();m++}
...
function SDiv(nm,sh){if(ns4&&nm.indexOf(".")>0)tD=self[nm]; else tD=gmstyle(nm);if(tD)if(sh)tD.visibility="visible";else {tD.visibility="hidden";if(ie55){gmif=gmobj("if"+nm);if(gmif)gmif.style.visibility='hidden';}}}
...
function popdn(){arg=popdn.arguments;if(arg[0]){if(arg[3].substr(0,5)!="show-"){if(ns4){shl(arg[2],arg[1],"hide")}else{arg[0].style.background=arg[4];arg[0].style.color=arg[5];if(arg[6])arg[0].style.borderColor=arg[6]}}else{if(ns4)omv=0;hlarr[hlcnt]=arg[2]+"@"+arg[1]+"@"+arg[4]+"@"+arg[5]+"@"+arg[6];hlcnt++;}menu=self[arg[2]];if(!menu[12]){clearTimeout(Mtimer);Mtimer=setTimeout("closeallmenus();",timegap)}}else{clearTimeout(Mtimer);Mtimer=setTimeout("closeallmenus()",timegap)}}
